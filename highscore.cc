 ...
 */
static char highscoreName[FILENAME_MAX];
...
static char highScoreDir[FILENAME_MAX] = { 0 };
static const char * hsc_directory()
{
#if __MACOSX__
    if(0 == highScoreDir[0]) {
        char * bp = SDL_GetPrefPath("Toast442.org", "Toppler");
        // Remove trailing slash, since everything else below assumes its missing
        bp[strlen(bp) - 1] = 0;
        strcpy(highScoreDir, bp);
        SDL_free(bp);
    }
...
static char highScoreFile[FILENAME_MAX] = { 0 };
static const char * hsc_scorefile()
{
    if(0 == highScoreFile[0]) {
        strcpy(highScoreFile, hsc_directory());
        strcat(highScoreFile, "/");
        strcat(highScoreFile, SCOREFNAME);
    }

    return highScoreFile;
}

static char highScoreLockFile[FILENAME_MAX] = { 0 };
static const char * hsc_scorelockfile()
{
    if(0 == highScoreLockFile[0]) {
        strcpy(highScoreLockFile, hsc_scorefile());
        strcat(highScoreLockFile, ".lck");
    }

    return highScoreLockFile;
}
...
static bool hsc_lock(void)
{
    ...
    if(globalHighscore) {
        ...
        while((lockfd = open(hsc_scorelockfile(), O_CREAT | O_RDWR | O_EXCL, S_IRUSR | S_IWUSR)) == -1) {
            dcl_wait();
            scr_swap();
        }
        close(lockfd);
...
void hsc_init(void)
{
...
#if __MACOSX__
    snprintf(highscoreName, 199, "%s", hsc_scorefile());
#else
    ...
    char fname[200];
    snprintf(fname, 200, "%s", hsc_scorefile());
    ...
    if(f) {
         ...
         */
        snprintf(fname, 200, "%s%i", hsc_scorefile(), rand());
        ...
        if(f) {
            ...
            snprintf(highscoreName, 200, "%s", hsc_scorefile());
            globalHighscore = true;
        }
    }

#endif // ifdef HISCOREDIR
